...
from autoencoders.adversarial import AdversarialAutoencoderMnist_war, AdversarialAutoencoderCifar10_war
from autoencoders.adversarial import AdversarialAutoencoderProstate, AdversarialAutoencoderafhq
...
def get_dataset(dataset):
    X_train = Y_train = X_test = Y_test = use_rgb = None
    if dataset == 'mnist':
        ...
        X_train = np.load("/content/gdrive/My Drive/Colab Notebooks/ABELE_prostate/ABELE_master/AfroMNIST/Ethiopic/Ethiopic_MNIST_X_train.npy")
        Y_train = np.load("/content/gdrive/My Drive/Colab Notebooks/ABELE_prostate/ABELE_master/AfroMNIST/Ethiopic/Ethiopic_MNIST_y_train.npy")
        X_test = np.load("/content/gdrive/My Drive/Colab Notebooks/ABELE_prostate/ABELE_master/AfroMNIST/Ethiopic/Ethiopic_MNIST_X_test.npy")
        Y_test = np.load("/content/gdrive/My Drive/Colab Notebooks/ABELE_prostate/ABELE_master/AfroMNIST/Ethiopic/Ethiopic_MNIST_y_test.npy")
        X_train = np.stack([gray2rgb(x) for x in X_train], 0)
        X_test = np.stack([gray2rgb(x) for x in X_test], 0)
        use_rgb = False
...
def get_autoencoder(X, ae_name, dataset, path_aemodels):
    ...
    if ae_name == 'aae':
        if dataset in ['mnist', 'fashion', 'cifar10bw', 'emnist', 'afromnist_e']:
        ...
        elif dataset in ['mnist_war']:
            ae = AdversarialAutoencoderMnist_war(shape=shape, input_dim=input_dim, latent_dim=latent_dim,
                                             hidden_dim=512, verbose=verbose, store_intermediate=store_intermediate,
                                             path=path_aemodels, name=name)
            ...
            ae = AdversarialAutoencoderCifar10(shape=shape, input_dim=input_dim, latent_dim=latent_dim,
                                               ...
                                               path=path_aemodels, name=name)
        elif dataset in ['cifar10_war']:
            ae = AdversarialAutoencoderCifar10_war(shape=shape, input_dim=input_dim, latent_dim=latent_dim,
                                               hidden_dim=128, verbose=verbose, store_intermediate=store_intermediate,
                                               path=path_aemodels, name=name)
        elif dataset in ['prostate']:
            ae = AdversarialAutoencoderProstate(shape=shape, input_dim=input_dim, latent_dim=latent_dim,
                                               hidden_dim=1152, verbose=verbose, store_intermediate=store_intermediate,
                                               path=path_aemodels, name=name)
        elif dataset in ['afhq']:
            ae = AdversarialAutoencoderafhq(shape=shape, input_dim=input_dim, latent_dim=latent_dim,
                                               hidden_dim=1024, verbose=verbose, store_intermediate=store_intermediate,
                                               path=path_aemodels, name=name)
        else:
            return -1
