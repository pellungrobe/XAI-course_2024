...
class Autoencoder(object):
    ...
    def encode(self, X):
        return self.encoder.predict(self.img_normalize(X)) #afhq
    ...
    def sample_images(self, epoch):
        ...
        label = self.discriminator.predict(z)
        ...
        for i in range(r):
            for j in range(c):
                if self.shape[2] == 1:
                    axs[i, j].imshow(Z[cnt].reshape(128,128), cmap='gray')
                    axs[i, j].set_title(str(np.round(label[cnt],2)), fontsize=6)
                else:
                    axs[i, j].imshow(Z[cnt].reshape(self.shape))
                    axs[i, j].set_title(str(np.round(label[cnt], 2)), fontsize=6)
                axs[i, j].axis('off')
                cnt += 1
        fig.savefig('%s%s_%d.png' % (self.path, self.name, epoch), dpi=1000)
        #fig.savefig('/content/gdrive/My Drive/Colab Notebooks/ABELE_prostate/ABELE_master/aemodels/prostate/aae/%s_%d.png' % (self.name, epoch))
        plt.close()
